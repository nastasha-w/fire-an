...
def get_roughdetmask(spectrum: fc.SpectrumFitBayes,
    ...
    # crosses cutlevel between index and index + 1
    crossinds = np.searchsorted(vvals, vcross) - 1
    print('enclinds: ', enclinds)
    print('crossinds: ', crossinds)
    for ai, encli in enumerate(enclinds):
        ...
        if ai == 0:
            if len(crossinds) == 0:
                cuti[0] = 0
            elif crossinds[0] < encli[0]:
                cuti[0] = np.max(crossinds[crossinds < encli[0]])
            else:
                cuti[0] = 0
        ...
        if ai == len(enclinds) - 1: 
            if len(crossinds) == 0:
                cuti[1] = len(vvals)
            elif crossinds[-1] > encli[1] - 1:
                cuti[1] = np.min(crossinds[crossinds > encli[1]]) + 1
            else:
                cuti[1] = len(vvals)
        ...
        if cuti[0] is None:
            ...
            cisel = np.logical_and(crossinds <= encli[0],
                                   crossinds > lastend)
            crossopts = crossinds[cisel]
            if len(crossopts) > 0:
                closestcross = np.max(crossopts)
                 # cutlevel crossing closest to component start
                cuti[0] = closestcross
            elif lastend == encli[0]: 
                cuti[0] = encli[0]
            else: # maximum between components
                cuti[0] = np.argmax(convflux[lastend : encli[0]])
                cuti[0] = cuti[0] + lastend + 1
        if cuti[1] is None:
            nextstart = enclinds[ai + 1][0]
            cisel = np.logical_and(crossinds + 1 < nextstart,
                                   crossinds >= encli[1])
            crossopts = crossinds[cisel]
            if len(crossopts) > 0: 
                closestcross = np.min(crossopts)
                cuti[1] = closestcross + 1
            elif encli[1] == nextstart:
                cuti[1] = nextstart
            else:
                cuti[1] = np.argmax(convflux[encli[1] : nextstart])
                cuti[1] = cuti[1] + encli[1] + 1
        cutinds.append(cuti)
...
def test_rough_detmasks(spectrum: fc.SpectrumFitBayes,
    ...
    for ci, (imin, imax) in enumerate(enclinds):
        ...
        vcen = 0.5 * (vvals[imin] + vvals[imax - 1])
...
def runtest_rough_detmasks():
    ...
    outname_temp = 'test_compdet_ne8_770_{specname}_lsf30_detlim13p5_maxb50.pdf'
